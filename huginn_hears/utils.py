...
class CustomLlamaCpp(LLM):
    ...
    Example:
        .. code-block:: python
            ...
            llm = LlamaCpp(repo_id="TheBloke/dolphin-2.6-mistral-7B-dpo-laser-GGUF", filename='*Q4_K_M.gguf')
    """
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            values["client"] = Llama.from_pretrained(repo_id=repo_id, filename=filename, **model_params)
        except Exception as e:
            raise ValueError(
                f"Could not load Llama model from: {repo_id=} and {filename=}. "
                f"Received error {e}"
            )
    ...
    def _identifying_params(self) -> Dict[str, Any]:
        ...
        return {**{"repo_id": self.repo_id, "filename": self.filename}, **self._default_params}
